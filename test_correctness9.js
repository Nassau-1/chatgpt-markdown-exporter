...
const mod = evaluate(codeOrig
  ...
  .replace(/const caption = finishDocument\(renderElementChildren\(node\)\)\.trim\(\) \|\| squashText\(node.textContent \|\| ""\);/g, 'const caption = renderElementChildren(node).trim() || squashText(node.textContent || "");')
);
