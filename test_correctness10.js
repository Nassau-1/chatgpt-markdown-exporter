...
const mod = evaluate(codeOrig
  .replace(/finishDocument\(renderElementChildren\(node\)\)\.trim\(\)/g, 'normalizeForDocument(renderElementChildren(node)).trim()')
  .replace(/finishDocument\(renderElementChildren\(cell\)\)/g, 'normalizeForDocument(renderElementChildren(cell))')
  .replace(/finishDocument\(proseChunks\.join\(""\)\.replace\(\/\\s\*\\n\\s\*\/g, " "\)\)\.trim\(\)/g, 'normalizeForDocument(proseChunks.join("").replace(/\\s*\\n\\s*/g, " ")).trim()')
  .replace(/const caption = finishDocument\(renderElementChildren\(node\)\)\.trim\(\) \|\| squashText\(node.textContent \|\| ""\);/g, 'const caption = normalizeForDocument(renderElementChildren(node)).trim() || squashText(node.textContent || "");')
);
