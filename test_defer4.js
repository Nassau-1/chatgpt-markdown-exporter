...
const codeMod = codeOrig
  .replace(/const title = finishDocument\(renderElementChildren\(node\)\)\.trim\(\);/, 'const title = renderElementChildren(node);')
  .replace(/const paragraph = finishDocument\(renderElementChildren\(node\)\)\.trim\(\);/, 'const paragraph = renderElementChildren(node);')
  .replace(/const quoted = finishDocument\(renderElementChildren\(node\)\)\.trim\(\);/, 'const quoted = renderElementChildren(node);')
  .replace(/return title \? \`\\\\n\\\\n\$\{"#"\.repeat\(level\)\} \$\{title\}\\\\n\\\\n\` : "";/, 'return title.trim() ? `\\n\\n${"#".repeat(level)} ${title.trim()}\\n\\n` : "";')
  .replace(/return paragraph \? \`\$\{paragraph\}\\\\n\\\\n\` : "";/, 'return paragraph.trim() ? `${paragraph}\\n\\n` : "";')
  .replace(/return \`\\\\n\\\\n\$\{quoted\.split\("\\\\n"\)\.map\(\(line\) => \`> \$\{line\}\`\)\.join\("\\\\n"\)\}\\\\n\\\\n\`;/,
  'return `\\n\\n${finishDocument(quoted).split("\\n").map((line) => `> ${line}`).join("\\n")}\\n\\n`;')
